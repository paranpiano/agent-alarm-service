...
- [ ] 1. 프로젝트 구조 및 공통 모듈 설정
  ...
  - [ ] 1.2 공유 데이터 모델 구현 (`server/models.py`)
...
- [ ] 2. Server 핵심 구현 (P0)
  ...
  - [ ] 2.5 LLM Service 구현 (`server/services/llm_service.py`)
  ...
  - [ ] 2.8 Flask API 라우트 구현 (`server/api/routes.py`)
...
- [ ] 4. Client GUI 구현 (P0)
  ...
  - [ ] 4.6 Client 진입점 구현 (`client/main.py`)
    - tkinter 앱 실행
    - Server URL 설정 (기본 http://localhost:8000)
    - _Requirements: 1.1_

- [ ] 5. 체크포인트 — Client-Server 통합 검증
  - Client GUI에서 테스트 이미지 전송 → Server 분석 → 결과 표시 흐름 확인
  - 모든 테스트 통과 확인, 사용자에게 질문이 있으면 확인

- [ ] 6. 판단 결과 저장/로그 구현 (P1)
  - [ ] 6.1 로그/저장 모듈 구현 (`server/logger.py`)
...
- [ ] 8. 이메일 알림 및 재시도 구현 (P2)
  - [ ] 8.1 이메일 알림 모듈 구현 (`server/services/email_notifier.py`)
