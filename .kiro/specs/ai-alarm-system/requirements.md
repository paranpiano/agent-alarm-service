...
- **Email_Notifier**: 알 수 없는 상황 발생 시 이메일 알림을 전송하는 모듈
...
**사용자 스토리:** 시스템 관리자로서, 예상하지 못한 상황이 발생하면 이메일 알림을 받을 수 있기를 원한다. 이를 통해 신속하게 대응할 수 있다.

#### 인수 조건

1. WHEN Unknown_Status 판단이 발생하면, THE Email_Notifier SHALL 지정된 수신자에게 알림 이메일을 전송한다
2. THE Email_Notifier SHALL 이메일에 판단 이유, 타임스탬프, 요청 식별자를 포함한다
3. IF 이메일 전송에 실패하면, THEN THE Email_Notifier SHALL 전송 실패를 로그에 기록하고 재시도를 최대 3회 수행한다
4. THE Email_Notifier SHALL 이메일 수신자 목록을 외부 설정 파일에서 로드한다
